...
function PhaseCard({
  ...
  onCompleteCourse, onSkipCourse, isBusy, userId, notes, dispatch,
        ...
        <div className="space-y-3 px-4 pb-6">
          {items.map((item, i) => (
            <CourseCard
              key={item.item_id}
              item={item}
              status={item.item_id === verifyingItemId ? 'pending_verification' : statuses[i]}
              ...
              dispatch={dispatch}
              verification={verifications[item.item_id]}
            />
          ))}
